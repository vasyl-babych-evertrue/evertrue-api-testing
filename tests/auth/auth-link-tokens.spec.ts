...
test.describe('Auth API - Link Token Authentication (Backend Services)', () => {
  ...
  test('should return 401 without App Token for Link Token creation', async ({ request }) => {
    ...
    // Without Authorization, API returns 401
    expect(response.status()).toBe(401);
  });

  test('should return 422 with invalid application name', async ({ request }) => {
    const response = await request.post('/auth/link_tokens', {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
    ...
    // Invalid application name returns 422
  ...
  test('should return 401 for invalid token exchange (fake app token)', async ({ request }) => {
    const response = await request.post('/auth/link_tokens', {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
    ...
    // Fake app token returns 401
    expect(response.status()).toBe(401);
  });

  test('should return 401 when providing both token and user_id with fake app token', async ({ request }) => {
    const response = await request.post('/auth/link_tokens', {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
    ...
    // Fake app token returns 401 before validation
    expect(response.status()).toBe(401);
  });
});
