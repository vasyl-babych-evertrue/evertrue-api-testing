...
test.describe('Properties API - Positive Flow', () => {
  ...
  // 7. Delete a non-default Contact property (DELETE /contacts/v1/properties/{id})
  test.describe('DELETE /contacts/v1/properties/{id} - Delete a non-default Contact property', () => {
  test('Status: Delete a non-default Contact property', async ({ request }) => {
    const id = createdPropertyId ?? 84581; // fallback to example id
    const response = await request.delete(`/contacts/v1/properties/${id}`, {
      params: { auth: authToken, auth_provider: AUTH_PROVIDER, oid: OID, app_key: config.headers.applicationKey },
      data: { updated_at: createdPropertyUpdatedAt ?? Date.now() },
    });
    expect([200, 204, 400, 404, 409, 422, 500]).toContain(response.status());
  });

  test('Schema: Delete a non-default Contact property', async ({ request }) => {
    const id = createdPropertyId ?? 84581;
    const response = await request.delete(`/contacts/v1/properties/${id}`, {
      params: { auth: authToken, auth_provider: AUTH_PROVIDER, oid: OID, app_key: config.headers.applicationKey },
      data: { updated_at: createdPropertyUpdatedAt ?? Date.now() },
    });
  ...
  test('Status: Delete a nested Contact property', async ({ request }) => {
    const id = createdNestedPropertyId ?? 84581;
    const response = await request.delete(`/contacts/v1/properties/nested/${id}`, {
      params: { auth: authToken, auth_provider: AUTH_PROVIDER, oid: OID, app_key: config.headers.applicationKey },
      data: { updated_at: createdNestedPropertyUpdatedAt ?? Date.now() },
    });
    expect([200, 204, 400, 404, 409, 422, 500]).toContain(response.status());
  });
